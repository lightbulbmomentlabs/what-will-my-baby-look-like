...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      successful: allUserImages?.filter(img => img.generation_success).length || 0,
      withUrls: allUserImages?.filter(img => img.original_image_url && img.original_image_url !== '').length || 0,
      successfulWithUrls: allUserImages?.filter(img => img.generation_success && img.original_image_url && img.original_image_url !== '').length || 0,
    };
